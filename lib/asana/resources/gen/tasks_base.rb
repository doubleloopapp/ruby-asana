  ...
  module Resources
    class TasksBase < Resource
      ...
      class << self
        ...
        # per_page - [int]  Results per page. The number of objects to return per page. The value must be between 1 and 100.
        # options - [Hash] the request I/O options
        # > opt_fields - [list[str]]  Defines fields to return. Some requests return *compact* representations of objects in order to conserve resources and complete the request more efficiently. Other times requests return more information than you may need. This option allows you to list the exact set of fields that the API should be sure to return for the objects. The field names should be provided as paths, described below. The id of included objects will always be returned, regardless of the field options.
        # > opt_pretty - [bool]  Provides “pretty” output. Provides the response in a “pretty” format. In the case of JSON this means doing proper line breaking and indentation to make it readable. This will take extra time and increase the response size so it is advisable only to use this during debugging.
        def get_tasks_for_project(client, project_gid: required("project_gid"), completed_since: nil, per_page: 20, options: { limit: 100 })
          path = "/projects/{project_gid}/tasks"
          path["{project_gid}"] = project_gid
          params = { completed_since:, limit: per_page }.reject { |_,v| v.nil? || Array(v).empty? }
          Collection.new(parse(client.get(path, params: params, options: options)), type: Task, client: client)
        end
